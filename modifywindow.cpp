    ...
    ui(new Ui::modifyWindow)
{
    ...
    QStringList In_school_list;
    if(p->School_status=="是")
        In_school_list<<"是"<<"否";
    else
        In_school_list<<"否"<<"是";
    ui->Schoolcombo->addItems(In_school_list);
    QStringList HealthCode_List;
    if(p->Health_Code=="绿码")
        HealthCode_List<<"绿码"<<"黄码"<<"红码";
    else if(p->Health_Code=="黄码")
        HealthCode_List<<"黄码"<<"绿码"<<"红码";
    else  HealthCode_List<<"红码"<<"绿码"<<"黄码";
    ui->Codecombo->addItems(HealthCode_List);
    QStringList acaList;
...
void modifyWindow::on_OKbtn_clicked()
{
    ...
    else
    {
        ...
        p->School_status=ui->Schoolcombo->currentText();
        p->Health_Code=ui->Codecombo->currentText();
