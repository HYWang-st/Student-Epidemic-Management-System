...
enum Sort{num_AscendingOrder,name_AscendingOrder,num_DescendingOrder,name_DescendingOrder,Healthcode};
    ...
    , ui(new Ui::MainWindow)
{
    ...
    academyName[3]="外文学院";
    academyName[4]="马克思学院";
    QFile file("../Student Epidemic Management System/nameList.txt");//内置数据
    if(!file.open(QIODevice::ReadOnly))
    {

        QMessageBox::question(this,"提示！","内置数据未成功初始化！请管理员登录后通过设置重新导入！",QMessageBox::Ok);
    }
    else
    {
...
void MainWindow::on_findbutton_clicked()
{
    ...
    if(!m_log->is_administrator){
        QMessageBox::question(this,"提示！","只有管理员才拥有此权限！",QMessageBox::Ok);
...
void MainWindow::on_stabutton_clicked()
{
    ...
    QProgressBar *probar_inschool=new QProgressBar(ui->sumWidget);
    QProgressBar *probar_not_inschool=new QProgressBar(ui->sumWidget);
    probar_inschool->setStyleSheet("QProgressBar{background:white;} QProgressBar::chunk{background:azure}");
    probar_not_inschool->setStyleSheet("QProgressBar{background:white;} QProgressBar::chunk{background:red}");
    ...
    ui->sumWidget->setHorizontalHeaderLabels(QStringList()<<"是否在校"<<"比例"<<"人数"<<"总人数");
    ui->sumWidget->setCellWidget(0,1,probar_inschool);
    ui->sumWidget->setCellWidget(1,1,probar_not_inschool);
    ui->sumWidget->setItem(0,0,new QTableWidgetItem("在校"));
    ui->sumWidget->setItem(1,0,new QTableWidgetItem("不在校"));
    int sum=0,inSchoolNum=0;
    Student *p=head;
    if(p==NULL){}
    else
    {
        while(p!=NULL)
        {
            if(p->School_status=="是")
                inSchoolNum++;
            sum++;
            p=p->next;
        }
    }
    probar_inschool->setMaximum(sum);
    probar_not_inschool->setMaximum(sum);
    probar_inschool->setValue(inSchoolNum);
    probar_not_inschool->setValue(sum-inSchoolNum);
    ui->sumWidget->setItem(0,2,new QTableWidgetItem(QString::number(inSchoolNum)));
    ui->sumWidget->setItem(1,2,new QTableWidgetItem(QString::number(sum-inSchoolNum)));
...
void MainWindow::on_addokbutton_clicked()
{
        ...
        else
        {
            ...
            current->School_status=ui->Schoolcombo->currentText();           
            current->Health_Code=ui->Codecombo->currentText();
...
void ctrl_acaWidget(Ui::MainWindow *ui,int AddOrMinus)
{
    ...
    else
    {
        while(p!=NULL)
        {
            if(p->School_status=="是"&&p->academy==academyName[curIndex])
            {
                inSchoolNum++;
                sum++;
            }
            else if(p->School_status=="否"&&p->academy==academyName[curIndex])
                sum++;
            p=p->next;
        }
    }
    QProgressBar *probar_inschool=new QProgressBar(ui->acaWidget);
    QProgressBar *probar_not_inschool=new QProgressBar(ui->acaWidget);
    probar_inschool->setStyleSheet("QProgressBar{background:white;} QProgressBar::chunk{background:azure}");
    probar_not_inschool->setStyleSheet("QProgressBar{background:white;} QProgressBar::chunk{background:red}");
    ...
    ui->acaWidget->setHorizontalHeaderLabels(QStringList()<<"在校情况"<<"比例"<<"人数"<<"总人数");
    ui->acaWidget->setCellWidget(0,1,probar_inschool);
    ui->acaWidget->setCellWidget(1,1,probar_not_inschool);
    ui->acaWidget->setItem(0,0,new QTableWidgetItem("在校"));
    ui->acaWidget->setItem(1,0,new QTableWidgetItem("不在校"));
    probar_inschool->setMaximum(sum);
    probar_not_inschool->setMaximum(sum);
    probar_inschool->setValue(inSchoolNum);
    probar_not_inschool->setValue(sum-inSchoolNum);
    ui->acaWidget->setItem(0,2,new QTableWidgetItem(QString::number(inSchoolNum)));
    ui->acaWidget->setItem(1,2,new QTableWidgetItem(QString::number(sum-inSchoolNum)));
...
void bubblesort(Student*p,int n,int sortMethod)
{
    ...
    else if(sortMethod==Healthcode)
    {
        for (int i = 0; i < n - 1; i++)
        {
            for (int j = 0; j < n - 1 - i; j++)
            {
                if (maps[p[j].Health_Code] < maps[p[j + 1].Health_Code] )
                {
                    Student temp = p[j];
                    p[j] = p[j + 1];
                    p[j + 1] = temp;
...
void cho_find(bool btnchoose,Ui::MainWindow *ui)
{
    ...
    QString flag_sel=ui->comboBox_sel->currentText();
     int i=0;//存数计数
            ...
            while (p!=NULL) {
                if (flag_college=="不限"){
                    if(flag_sex=="不限"){
                       ...
                       selected[i].Health_Code=p->Health_Code;
                       i++;
                    }
                }
                if(flag_sex=="不限"){
        ...
        if(flag_sel=="健康码")bubblesort(selected,i,Healthcode);//按照映射方式排序
        //控制输出
        ui->del_tableWidget->setRowCount(0);
        if(i==0)
        {
        ...
        else
        {
            ...
            ui->del_tableWidget->setHorizontalHeaderLabels(QStringList()<<" "<<"姓名"<<"性别"<<"学号"<<"学院"<<"健康码");
             for(int j=0;j<i;j++){
                 ...
                 pitem->setFlags(pitem->flags()&(~Qt::ItemIsEditable));
                 ui->del_tableWidget->setItem(rowIndex,5,new QTableWidgetItem(selected[j].Health_Code));
...
void Export()//退出前导出文件
{
    QFile file("../Student Epidemic Management System/nameList.txt");
    if(!file.open(QIODevice::WriteOnly))
    {
        ...
        while(p!=NULL)
        {
            stream<<p->name<<" "<<p->sex<<" "<<p->num<<" "<<p->academy<<" "<<p->telenum<<" "
                 <<p->address<<" "<<p->email<<" "<<p->QQ<<" "<<p->VX<<" "<<p->School_status<<" "
                 <<p->Health_Code<<" "<<'\r'<<'\n';
        p=p->next;
        }
        stream<<"END ";
...
void data_lead_in(QString str)
{
    ...
    while (1)
    {
        ...
        current->QQ=word[7];
        current->VX=word[8];
        current->School_status=word[9];
        current->Health_Code=word[10];
